...
/**
 * Clean and robust JSON parsing that handles markdown wrapping, extra text boundaries,
 * and trailing commas.
 */
function cleanAndParseJson(text: string): any {
  if (!text) {
    throw new Error("Empty JSON text received.");
  }
  ...
  try {
    ...
    // 2. Clear Markdown backticks if present
    if (cleaned.startsWith("```")) {
      ...
      try {
        ...
        // Proceed to substring matching
      }
    }

    // 3. Match from first object symbol to last object symbol
    ...
    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
      ...
      try {
        ...
        try {
          // Remove potential trailing commas before closing braces/brackets inside simple scopes
          const sanitized = candidate.replace(/,\s*([}\]])/g, "$1");
          return JSON.parse(sanitized);
        } catch (e4) {
          // Proceed to bracket matching
        }
      }
    }
    ...
    if (firstBracket !== -1 && lastBracket !== -1 && lastBracket > firstBracket) {
      ...
      try {
        ...
        try {
          ...
          // Proceed of throwing original error
        }
      }
    }
...
app.post("/api/parse-resume", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const parsedResumeObj = cleanAndParseJson(parsedJsonText);
...
app.post("/api/analyze-match", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json(cleanAndParseJson(matchJsonText));
  } catch (error: any) {
